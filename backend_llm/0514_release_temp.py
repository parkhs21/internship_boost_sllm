...
def generate_stream():
    ...
    prompt = lambda input_text: f"""[[[2]{{"role": "system", "content": "Your role is to provide concise and accurate answers based on the inputs you receive. You have to cover a wide range of topics, from programming to general knowledge questions. Summarize your points first, and deliver the necessary information concisely. Make sure you have answered the questions clearly related to programming. Do not repeat the questions in your answer, but provide them in a structured and clear format. Answer only once per question.You should not create your own "role": "system", "content"."
    }}]],
    [[[1]{{"role": "user", "content": "{input_text}"
    }}]],
    [[[0]{{"role": "system", "content": "
    """
    
    streamer = TextIteratorStreamer(tokenizer, skip_prompt=True)
    inputs = tokenizer([prompt(input_text)], return_tensors="pt").to("cuda:1")
    
    kwargs = dict(input_ids=inputs["input_ids"], streamer=streamer, max_new_tokens=max_new_token, do_sample=False)
    if do_sample:
